...
function setUserInfoCookie(key, value) {
  const encodedPair = encodeURIComponent(`${key}:${value}`);
  const expires = new Date(Date.now() + 10000).toUTCString();
  document.cookie = `userInfo=${encodedPair}; expires=${expires}; path=/;`;
  console.log('Cookie userInfo has been set successfully.');
}
